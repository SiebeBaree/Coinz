...
function getCommands(commandsList: CommandList, category: string, searchTerm: string) {
    if (searchTerm) {
        return Object.keys(commandsList).filter((command) =>
            command.toLowerCase().includes(searchTerm.toLowerCase().replace('/', '')),
        );
...
function CommandCard({ name, command }: { name: string; command: Command }) {
        ...
        >
            ...
            <div className="flex flex-col justify-center flex-grow">
                ...
                >
                    ...
                    <ChevronDownIcon
                        className="fill-primary h-12 w-12 transition-all duration-300 ease-in-out"
                        style={{
